...
if __name__ == "__main__":
    ...
    logging.config.fileConfig(
        ...
        defaults={"logfilename":os.path.join(curdir, "logging", str(job_id), str(threading.current_thread().ident), f"logs_{eval_method}_{valid_proj[:-4]}.log")}
    )
    ...
    def hyper_param_opt():
        ...
        for _ in range(nbr_tries): 
            ...
            if eval_method=="within":
                ...
                scores[final_eval_score] = f"max_depth={max_depth} lr={lr} epsilon={epsilon} gamma={gamma} batch_size={BATCH_SIZE} n_episodes={n_episodes} seed={seed} output={threading.current_thread().ident}"
            else:
    ...
    for n in range(nbr_tries):
        t = Thread(target=hyper_param_opt, args=())
        threads.append(t)
        t.start()
